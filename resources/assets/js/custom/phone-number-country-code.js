...
function loadPhoneNumberCountryCode() {
    ...
    // Get the default country from data attribute or use Saudi Arabia
    let defaultCountry = input.getAttribute('data-initial-country') || 'sa';
    
    // initialise plugin
    let intl = window.intlTelInput(input, {
        initialCountry: defaultCountry, // Set default country
        separateDialCode: true,
        utilsScript: '../../public/assets/js/inttel/js/utils.min.js',
    });

    // Force set the country multiple times to ensure it sticks
    setTimeout(function() {
        intl.setCountry(defaultCountry);
    }, 100);
