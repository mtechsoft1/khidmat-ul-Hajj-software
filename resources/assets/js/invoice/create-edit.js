...
function loadSelect2ClientData() {
    ...
    $("#client_id,#status,#templateId,#invoiceCategoryId").select2();
    $(".discount-type").select2({
        width: "200px",
    });
}

function toggleHotelReservationFields() {
    if (!$("#invoiceCategoryId").length || !$("#hotelReservationFields").length) {
        return;
    }

    let selectedText = $("#invoiceCategoryId option:selected").text() || "";
    let isHotelReservation = selectedText.trim().toLowerCase() === "hotel reservation";
    $("#hotelReservationFields").toggleClass("d-none", !isHotelReservation);
}

listenChange("#invoiceCategoryId", function () {
    toggleHotelReservationFields();
});
...
listenClick("#saveAsDraft,#saveAndSend", function (event) {
    ...
    if (invoiceStates == 1) {
        swal({
            title: Lang.get("messages.invoice.send_invoice") + " !",
            text: Lang.get("messages.invoice.are_you_sure_send"),
            icon: "warning",
            buttons: {
                confirm: Lang.get("messages.invoice.yes_send"),
    ...
    if (invoiceStatus == 1) {
        swal({
            ...
            buttons: [
                ...
                Lang.get("messages.invoice.yes_send"),
            ],
        }).then(function (willSend) {
            if (willSend) {
                screenLock();
